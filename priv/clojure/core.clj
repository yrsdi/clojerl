...
(def
  ...
  fn (fn* fn [&form _&env & decl]
         (let [x (cons 'fn* (new clojerl.List decl))]
           (clj_core:with_meta x (clj_core:meta &form)))))
...
(def
  ^{:arglists '([coll])
    ...
    :added "1.0"}
  first (fn first [coll] (clj_core:first coll)))
...
(def
  ...
  argument.  If there are no more items, returns nil."
    :added "1.0"}
  next (fn next [x] (clj_core:next x)))
...
(def
  ...
  argument."
    :added "1.0"}
  rest (fn rest [x] (clj_core:rest x)))
...
(def
  ^{:arglists '([coll])
    ...
    :tag clojerl.ISeq
    :added "1.0"}
  seq (fn seq [coll] (clj_core:seq coll)))
...
(def
  ...
  satisfies? (fn satisfies? [protocol x]
               (clj_core:satisfies? protocol (clj_core:type x))))

(def
  ^{:arglists '([atype x])
    :doc "Returns true if x's type is atype"
    :added "1.0"}
  instance? (fn instance? [atype x] (erlang:== atype (clj_core:type x))))
...
(def
  ^{:arglists '([x])
    ...
    :added "1.0"}
  char? (fn char? [x]
          (erlang:and (erlang:is_binary x)
                          (erlang:=:= 1 (erlang:size x)))))
...
(def
  ...
  assoc
  (fn assoc
    ([map key val] (clj_core:assoc map key val))
    ([map key val & kvs]
    ...
    (let [asig
          (fn [fdecl]
            (let [arglist (first fdecl)
                  ...
                  arglist (if (clj_core:equiv '&form (first arglist))
                            (clj_core:subvec arglist 2 (clj_core:count arglist))
                            arglist)
                  body (next fdecl)]
              (if (map? (first body))
                (if (next body)
...
(defn vector
  ...
  ([a b c d] [a b c d])
  ([a b c d & args]
   (clj_core:vector (cons a (cons b (cons c (cons d args)))))))
...
(defn str
  ...
  (^clojerl.String [^Object x]
                    (if (nil? x) "" (clj_core:str x)))
  (^clojerl.String [x & ys]
                    ((fn [acc more]
                       (if more
                         (recur (clojerl.String:append acc (str (first more)))
                                (next more))
                         acc))
                     (clj_core:str x) ys)))
...
(defn symbol
  ...
  {:tag clojerl.Symbol
   :added "1.0"}
  ([name] (if (symbol? name) name (clj_core:symbol name)))
  ([ns name] (clj_core:symbol ns name)))
...
(defn gensym
  ...
  {:added "1.0"}
  ([] (gensym "G__"))
  ([prefix-string] (clj_core:gensym prefix-string)))
...
(defn keyword
  ...
  ([name] (cond (keyword? name) name
                (symbol? name) (new clojerl.Keyword (clj_core:namespace name)
                                                    (clj_core:name name))
...
(defn find-keyword
  ...
  {:tag clojerl.Keyword
   :added "1.3"}
  ([name] (cond (keyword? name) name
                (symbol? name) (clojerl.Keyword:find (clj_core:namespace name)
                                                       (clj_core:name name))
...
(defn =
  ...
  {:added "1.0"}
  ([x] true)
  ([x y] (clj_core:equiv x y))
  ([x y & more]
   (if (clj_core:equiv x y)
     (if (next more)
       (recur y (first more) (next more))
       (clj_core:equiv y (first more)))
     false)))
...
(defn nth
  ...
  {:added "1.0"}
  ([coll index] (clj_core:nth coll index))
  ([coll index not-found] (clj_core:nth coll index not-found)))
...
(defn contains?
  ...
  {:added "1.0"}
  [coll key] (clj_core:contains? coll key))
...
(defn get
  ...
  ([map key not-found]
   (clj_core:get map key not-found)))
...
(defn select-keys
  ...
  [map keyseq]
    (loop [ret {} keys (seq keyseq)]
      (if keys
        (let [entry (clj_core:find map (first keys))]
          (recur
           (if entry
             (conj ret entry)
             ret)
...
(defn keys
  ...
  {:added "1.0"}
  [map] (clj_core:keys map))

(defn vals
  "Returns a sequence of the map's values, in the same order as (seq map)."
  {:added "1.0"}
  [map] (clj_core:vals map))
...
(defmacro defmulti
  ...
  (let [docstring   (if (string? (first options))
    ...
    (let [options   (apply hash-map options)
      ...
      `(let [v# (def ~mm-name)]
         (when-not (and (clojerl.Var:has_root v#)
                        (instance? clojerl.MultiFn (deref v#)))
           (defn ~(with-meta mm-name m)
             [& args#]
             (let [val# (apply ~dispatch-fn args#)
                   f#    (clojerl.MultiFn:get_method ~(name mm-name) val# ~default ~hierarchy)]
               (when (nil? f#)
...
(defmacro defmethod
  ...
  (let [fn-name (symbol (str (name multifn) "_method_" dispatch-val))]
    `(do
       ...
       (erl-on-load*
        (clojerl.MultiFn:add_method ~(name multifn)
                                      ~dispatch-val
                                      (var ~fn-name))))))
...
(defn remove-all-methods
  ...
  {:added "1.2"}
  [^Var multifn]
  (clojerl.MultiFn:remove_all (name multifn)))

(defn remove-method
  "Removes the method of multimethod associated with dispatch-value."
  {:added "1.0"}
  [^clojure.lang.MultiFn multifn dispatch-val]
  (clojerl.MultiFn:remove_method multifn dispatch-val))
...
(defn methods
  ...
  (clojerl.MultiFn:get_method_table multifn))
...
(defn get-method
  ...
  {:added "1.0"}
  [^clojure.lang.MultiFn multifn dispatch-val]
  (clojerl.MultiFn:get_method multifn dispatch-val))
...
(defn find-var
  ...
  {:added "1.0"}
  [sym] (clj_namespace:find_var sym))
...
(defn deref
  ...
  ([ref]
   (if (satisfies? clojerl.IDeref ref)
     (clj_core:deref ref)
     (throw (str "unimplemented deref for type " (clj_core:type ref)))
     #_(deref-future ref)))
...
(defn sort
  ...
  ([comp coll]
   (if (seq coll)
     (lists:sort (clj_core:to_list coll))
     ())))
...
(defn read
  ...
  ([opts stream]
   (clj_reader:read ""
                      (clojerl.Map:to_erl_map (assoc opts :io-reader stream)))))
...
(defn read-string
  ...
  {:added "1.0"}
  ([s] (clj_reader:read s))
  ([opts s] (clj_reader:read s (clojerl.Map:to_erl_map opts))))
  ...
  (cond
    ...
    (symbol? (bindings 0)) `(let ~(subvec bindings 0 2)
                              (try
                                ...
                                (finally
                                  (erlang.io.Closeable:close ~(bindings 0)))))
...
(defn macroexpand-1
  ...
  {:added "1.0"}
  [form]
  (clj_analyzer:macroexpand_1 nil form))
...
(defn set
  ...
  (if (set? coll)
    (with-meta coll nil)
    (clj_core:hash_set (clj_core:to_list coll))))
...
(defn create-ns
  ...
  {:added "1.0"}
  [sym]
  (clj_namespace:find_or_create sym))
...
(defn ns-name
  ...
  {:added "1.0"}
  [ns]
  (clj_namespace:name (the-ns ns)))

(defn ns-map
  "Returns a map of all the mappings for the namespace."
  {:added "1.0"}
  [ns]
  (clj_namespace:get_mappings (the-ns ns)))

(defn ns-unmap
  "Removes the mappings for the symbol from the namespace."
  {:added "1.0"}
  [ns sym]
  (clj_namespace:unmap (the-ns ns) sym))
    ...
    (doseq [name (map name to-do)]
      (when-not (exclude name)
        (let [v   (nspublics name)
          ...
          (clj_namespace:refer *ns* (or (rename sym) sym) v))))))
...
(defn alias
  ...
  {:added "1.0"}
  [alias namespace-sym]
  (clj_namespace:add_alias *ns* alias (the-ns namespace-sym)))

(defn ns-aliases
  "Returns a map of the aliases for the namespace."
  {:added "1.0"}
  [ns]
  (clj_namespace:get_aliases (the-ns ns)))

(defn ns-unalias
  "Removes the alias for the symbol from the namespace."
  {:added "1.0"}
  [ns sym]
  (clj_namespace:remove_alias (the-ns ns) sym))
...
(defn ns-resolve
  ...
  ([ns env sym]
   (when-not (contains? env sym)
     (clj_namespace:find_var (the-ns ns) sym))))
                     ...
                     (let [gmap (gensym "map__")
                       ...
                       (loop [ret (-> bvec (conj gmap) (conj v)
                                      (conj gmap) (conj `(if (seq? ~gmap)
                                                           (new clojerl.Map (clj_core:to_list ~gmapseq))
                                                           ~gmap))
                                      ((fn [ret]
                                         (if (:as b)
                                           (conj ret (:as b) gmap)
...
(defn re-run
  ...
  {:added "1.0"}
  [re s & opts]
  (let [opts (clj_core:to_list opts)
        res  (erlang.util.Regex:run re s opts)]
  ...
  (let [process-reference
        ...
        gen-class-call
          (when gen-class-clause
            (list* `gen-class
                   :name (binary:replace (str name) "-" "_")
                   :impl-ns name
...
(defmacro defonce
  ...
  [name expr]
  `(let [v# (def ~name)]
     (when-not (clojerl.Var:has_root v#)
       (def ~name ~expr))))
...
(defn- throw-if
  ...
  (when pred
    (let [message   (apply format fmt (map str args))
      ...
      (erlang:raise :error message raw-trace))))
...
(defn- root-resource
  ...
  (str "/"
       (-> (name lib)
           (binary:replace "." "/" '#erl (:global))
           (binary:replace "-" "_" '#erl (:global)))))
...
(defn- load-one
  ...
  (when require
    (clj_core:set! #'*loaded-libs* (conj *loaded-libs* lib))))
...
(defn pmap
  ...
  {:added "1.0"}
  ([f coll]
   (let [n (+ 2 (erlang:system_info :logical_processors_available))
...
(def ^:dynamic *clojure-version*
  (let [properties (second (application:get_all_key :clojerl))
        ...
        (re-matches
         ...
         version-string)
        clojure-version {:major       (erlang:binary_to_integer major)
                         :minor       (erlang:binary_to_integer minor)
                         :incremental (erlang:binary_to_integer incremental)
                         :qualifier   (if (= qualifier "SNAPSHOT") nil qualifier)}]
    (if (clojerl.String:contains version-string "SNAPSHOT")
